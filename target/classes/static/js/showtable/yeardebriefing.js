...
function build_select() {
    $.ajax({
        url: "/edumanagement/teacher/select_debriefing_year",
        type: "get",
        success: function (result) {
            ...
            if(year == ""){
                ...
                // window.location.href="year _debriefing"
            }
            //默认页面年份查询
            a(year[0].year)
...
function a(year) {
    $.ajax({
        url: "/edumanagement/teacher/select_debriefing_year_info",
        type: "get",
        async: false,
        dataType: 'json',
        data: {
...
$("#herf").click(function () {
    var year = $("#year").val()
    window.location.href = localhostPath + projectName + "/static/down_year_debriefing?year="+year;
})
...
$("#dayin").click(function () {
    ...
    window.open('/edumanagement/teacher/print_year_debriefing?year='+year,'_blank','width=800,height=auto,menubar=no,toolbar=no,location=no,directories=no,status=no,scrollbars=yes,resizable=yes')
...
$("#submit").click(function () {
    if ($(this).attr("flag" ) == 1) {       //判断修改按钮属性值 1：修改   2：保存
        ...
        var teachingTask = '';
        $(".aa input[type=text]").each(function () {
            if (this.value != "") {
